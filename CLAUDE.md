...
`PLA_spine_v15.STL`은 외부 수정본이라 불리언 잔여물이 있다: **5개 컴포넌트**, foot boss에
**비다양체 에지 10개**. 유지 comp0(35,580mm³)+comp3(282.8), 폐기 = 중복 평면 슬랩 쌍
(+/− 부호 반대, 거의 상쇄) + 뒤집힌 내부 공동 셸. robotics_lab은 렌더만 하면 되니 원본 그대로
쓰지만 PhysX는 질량·충돌을 계산해야 해서 여기서 제거한다. **빌드가 매번 폐기 목록을 출력한다.**
외곽 실루엣은 안 변한다. [[design-rebuild-not-append]] 패턴 그대로이므로 CAD 기록용
클린 재익스포트는 여전히 받을 만하다.
...
- 인필률 확정 → 질량 재빌드. robotics_lab도 `ft identify` 재실행을 빚지고 있다.
